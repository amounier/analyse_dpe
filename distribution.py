...
def fit_dpe_data(dep_code, method='curve_fit', old_built_filter=False, verbose=True):
    ...
    r2_value : float
        Coefficient de régression entre # todo : completer
    param : tuple de floats # todo: vérifier
    ...
    nb_dpe = len(dpe_data)
    counter_dict = dict(dpe_data.conso_5_usages_ep_m2.value_counts())
    counter_dict_sorted = {k: v for k, v in sorted(counter_dict.items(), key=lambda item: item[0])} # todo : suppr car inutile ?
    ...
    if method=='curve_fit':
        ...
        # todo: enlever dep_code car lourd pour rien ? 'dep_code':dep_code,
...
def plot_dpe_distribution(path, dep_code, save, plot_mean, plot_median, window_size, plot_fit=False, plot_curve_fit=True, old_built_filter=False, max_xlim=600) :
    ...
    if old_built_filter:
        ax.set_title(f"Logements construits avant 1974. Taille de l'échantillon : {nb_dpe} DPE", fontsize=10)
    else : 
...
def calcul_bunching(dep_code, method, itv_bunching, window_size, plot_ecart, path, old_built_filter, max_xlim = 600, verbose=False):
    ...
    bunching : panda DataFrame 
        Bunching pour chaque seuil :  A/B  |  B/C  |  C/D  |  D/E  |  E/F  |  F/G
    ...
    if method=='AMP': # méthode "Average Manipulation Density" (Civel et al.).
        ...
        for k, seuil in etiquette_ep_seuils.items():
            ...
            AMP = (nb_gauche - nb_droite) / (nb_gauche+nb_droite)  # average manipulation density autour du seuil # on pourrait aussi diviser par nb tot DPE (AJa et al) ? 
            # AMP = (nb_gauche - nb_droite) / nb_dpe  # average manipulation density autour du seuil # on pourrait aussi diviser par nb tot DPE (AJa et al) ? 
...
# =============================================================================
        ...
        for k, seuil in etiquette_ep_seuils.items():
            ...
            # nb_dpe_seuil = dpe_data_df[(dpe_data_df.conso_5_usages_ep_m2_arrondie > seuil_gauche) & (dpe_data_df.conso_5_usages_ep_m2_arrondie <= seuil_droite)].nb_observations.sum()
            nb_dpe_seuil = dpe_data_df[(dpe_data_df.conso_5_usages_ep_m2_arrondie > seuil-itv_bunching) & (dpe_data_df.conso_5_usages_ep_m2_arrondie <= seuil + itv_bunching)].nb_observations.sum()
            
            # Ajout d'une colonne correspondante au seuil dans le bunching DataFrame 
            bunching_df[f'{k}_method_{method}'] = dpe_data_df_filtered['y_diff_moyenne_norm_abs'].sum() #*nb_dpe/nb_dpe_seuil
